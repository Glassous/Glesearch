...
const convertMode = ref('xml') // 'xml' | 'formatted' 转换模式下显示的内容类型
...
const formatXML = async () => {
  ...
  try {
    // 简单的XML格式化处理
    let formatted = xmlContent.value
    ...
    // 添加换行和缩进
    let indentLevel = 0
    const indentStr = ' '.repeat(xmlOptions.value.indent)
    
    formatted = formatted.replace(/<\/?[^>]+>/g, (match) => {
      if (match.startsWith('</'))
        indentLevel--
      
      const result = '\n' + indentStr.repeat(Math.max(0, indentLevel)) + match
    ...
    formattedContent.value = formatted.trim()
  } catch (error) {
    console.error('XML格式化失败:', error)
    errorMessage.value = 'XML格式处理失败，请检查语法'
  } finally {
    loading.value = false
  }
}
...
const handleFileUpload = (event) => {
  ...
  const validExtensions = ['.xml', '.xhtml', '.svg']
  const isValidFile = validExtensions.some(ext => file.name.toLowerCase().endsWith(ext))
  
  if (!isValidFile) {
    errorMessage.value = '请选择.xml、.xhtml或.svg文件'
    return
  }
...
const downloadXML = () => {
  ...
  link.download = uploadedFileName.value || `xml_${Date.now()}.xml`
  link.href = url
...
const handleInput = () => {
  if (viewMode.value === 'split') {
    clearTimeout(window.xmlFormatTimeout)
    window.xmlFormatTimeout = setTimeout(formatXML, 300)
  }
}
...
const switchViewMode = (mode) => {
  viewMode.value = mode
  if (mode === 'split' && xmlContent.value.trim()) {
    formatXML()
  }
}
...
<template>
  ...
  <main class="main-content">
      ...
      <div class="options-form">
        ...
        <div class="options-grid">
          <label class="checkbox-label">
            <input v-model="xmlOptions.sortAttributes" @change="formatXML" type="checkbox">
            <span>排序属性</span>
          </label>
          <label class="checkbox-label">
            <input v-model="xmlOptions.validateSyntax" @change="formatXML" type="checkbox">
            <span>语法验证</span>
          </label>
          <div class="indent-control">
            <label>缩进空格数：</label>
            <select v-model="xmlOptions.indent" @change="formatXML">
    ...
    <section class="editor-preview-section">
      <h3>✏️ {{ viewMode === 'split' ? 'XML编辑器 & 预览' : 'XML编辑器' }}</h3>
      ...
      <div v-if="viewMode === 'convert'" class="convert-mode">
        <div class="editor-form">
          ...
          <div class="convert-switcher">
            <button 
              :class="['convert-btn', { active: convertMode === 'xml' }]"
              @click="switchConvertMode('xml')"
            >
            ...
            >
              <span class="convert-icon">🏷️</span>
              <span>格式化预览</span>
            </button>
          </div>
          ...
          <textarea 
            ...
            :placeholder="convertMode === 'xml' ? '在此编辑XML...' : 'XML格式化显示区域'"
          ...
          <div class="editor-actions">
            <button @click="formatXML" class="btn-primary" :disabled="loading">
              {{ loading ? '⏳ 处理中...' : '🔄 格式化' }}
            </button>
            <button @click="downloadXML" class="btn-secondary" :disabled="!xmlContent.trim()">
              💾 下载
            </button>
          </div>
        </div>
...
.mode-switcher, .convert-switcher {
  display: flex;
  gap: 1rem;
  margin-bottom: 1rem;
}

.mode-btn, .convert-btn {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.75rem 1rem;
...
.mode-btn:hover, .convert-btn:hover {
  background: var(--glass-hover);
  transform: translateY(-2px);
  box-shadow: 0 4px 16px var(--glass-shadow);
}

.mode-btn.active, .convert-btn.active {
