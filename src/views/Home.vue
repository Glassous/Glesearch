...
import QuerySection from '@/components/query/QuerySection.vue'
import ToolsSection from '@/components/tools/ToolsSection.vue'
import EntertainmentSection from '@/components/entertainment/EntertainmentSection.vue'
...
const selectedEngine = ref(localStorage.getItem('selectedEngine') || 'glassous')
...
const engineIcons = {
  ...
  glassous: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24px" height="24px"><path fill="#4285f4" d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>',
...
const handleSearch = () => {
  ...
  // 如果是Glassous搜索引擎，跳转到内部搜索页面
  if (selectedEngine.value === 'glassous') {
    router.push({
      ...
      <div class="search-box">
        ...
        <div class="search-engine-selector" @click="switchEngine">
          <div class="engine-display">
            <div v-if="selectedEngine === 'glassous'" class="engine-icon-svg" :title="currentEngine.name">
              <img src="/src/assets/images/Glassous-search.png" alt="Glassous" style="width: 24px; height: 24px; border-radius: 3px;" />
            </div>
            <div v-else-if="engineIcons[selectedEngine]" class="engine-icon-svg" v-html="engineIcons[selectedEngine]" :title="currentEngine.name"></div>
          ...
          <div v-if="showEngineDropdown" class="engine-dropdown">
            ...
            >
              <div v-if="engine.id === 'glassous'" class="engine-option-icon">
                <img src="/src/assets/images/Glassous-search.png" alt="Glassous" style="width: 20px; height: 20px; border-radius: 3px;" />
              </div>
              <div v-else class="engine-option-icon" v-html="engineIcons[engine.id]"></div>
